...
export default function TransportBrokerRateEnquiryDetailPage() {
  ...
  const [enquiry, setEnquiry] = useState<TransportBrokerRateEnquiry | null>(
    null
  );
  ...
  const fetchEnquiry = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/api/transport-broker-rate-enquiries/${id}`,
        {
          headers: {
      ...
      if (data.routeId) {
        const routeResponse = await fetch(
          `http://localhost:3001/api/transport-routes/${data.routeId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const fetchBids = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://localhost:3001/api/transport-rate-bids?enquiryId=${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const sortedBids = [...bids].sort((a, b) => {
    ...
    switch (sortBy) {
      case "rate":
        ...
        break;
      case "broker":
        compareValue = (a.broker?.companyName || "").localeCompare(
  ...
  const fetchBrokers = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://localhost:3001/api/brokers?limit=100`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        "http://localhost:3001/api/transport-rate-bids",
        {
          method: "POST",
          headers: {
  ...
  const handleInlineSave = async (bidId: number) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://localhost:3001/api/transport-rate-bids/${bidId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        "http://localhost:3001/api/transport-orders",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const getRouteDisplay = () => {
    ...
    if (route.locations && route.locations.length > 0) {
      const sortedLocations = [...route.locations].sort(
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-black">
            Transport Rate Enquiry Details
          </h1>
          {getStatusBadge(enquiry.status)}
        </div>
        <button
          onClick={() =>
            router.push(`/transport-broker-rate-enquiries/${id}/edit`)
          }
      ...
      {/* WhatsApp Bid Requests Section */}
      <BidRequestsTable enquiryId={parseInt(id)} isExpanded={true} />
                    ...
                    >
                      ...
                      {brokers.map((broker) => (
                        <option key={broker.id} value={broker.id}>
                          {broker.companyName} - {broker.personName} (
                          {broker.city})
                        </option>
                      ))}
                    </select>
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-xs">
                        {editingBidId === bidWithBroker.bid.id ? (
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              onClick={() =>
                                handleInlineSave(bidWithBroker.bid.id)
                              }
            ...
            <div className="bg-white rounded-lg max-w-md w-full p-6">
              ...
              <div className="space-y-3 mb-4">
                <div className="text-sm">
                  ...
                  <p className="font-medium text-black">
                    {selectedBid.broker?.companyName}
                  </p>
                </div>
                <div className="text-sm">
                  <p className="text-gray-600">Rate:</p>
                  <p className="font-medium text-black">
                    ₹{selectedBid.bid.rate.toLocaleString()}
                  </p>
                </div>
                <p className="text-sm text-gray-600">
                  This will create an order for this enquiry with the selected
                  bid.
                </p>
              </div>
